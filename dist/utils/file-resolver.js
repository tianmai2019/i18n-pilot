...
function buildGlobPatterns(extensions) {
    if (extensions.length === 1) {
        return [`**/*.${extensions[0]}`];
    }
    return [`**/*.{${extensions.join(',')}}`];
}
async function loadI18nIgnore(rootDir) {
...
export async function resolveFiles(options) {
    const { targetPath, extensions = DEFAULT_EXTENSIONS, ignorePatterns = [], useI18nIgnore = true, } = options;
